...
public class ApiService implements IApiService {
	...
	public void updateSession(Session session, String authData, long timestamp) {
		...
		java.util.List<Session> oldSession = sessionService.getSessions(device,
				1, 0);
		...
		long timeout = now + (Integer) applicationConfig.get("general")
				.get("api_timeout");
	 ...
	 */
	@Transactional(value = "rentTxManager", propagation = Propagation.SUPPORTS, readOnly = false, rollbackFor = java.lang.Throwable.class)
	public void requestSession(Session session, String authData,
			long timestamp, List<Integer> roles) {
		String deviceId = session.getDeviceId();
		session.genId();
		//
		// get device from database.
		...
		timeout += (Integer) applicationConfig.get("general")
				.get("api_timeout");
		session.setTimeout(timeout);
	}
